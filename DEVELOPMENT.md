...
## Build the Khulnasoft Provider

The Khulnasoft Terraform provider can be pulled from the [Hashicorp registry](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest) as for the included examples in this repository.
...
```
git clone https://github.com/khulnasoft/terraform-provider-khulnasoft.git
...
The last command will compile the Terraform Provider and generate a `terraform-provider-khulnasoft` binary in your local directory and then will install into the folder containing the Terraform resources.
...
## Test Khulnasoft Terraform Provider
...
Finally, make sure to replace the Khulnasoft credentials and URL in the `provider` block and start trying out the different resources as provided in the example file.
