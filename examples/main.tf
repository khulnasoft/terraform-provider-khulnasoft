...
provider "khulnasoft" {
  username = var.khulnasoft_username
  password = var.khulnasoft_password
  khulnasoft_url = var.khulnasoft_url
}
...
resource "khulnasoft_application_scope_saas" "terraformiap" {
  description = "khulnasoft application scope saas"
  name        = "khulnasoft_application_scope_saas"
...
resource "khulnasoft_khulnasoft_api_key" "terraform_api_key" {
...
resource "khulnasoft_khulnasoft_label" "khulnasoft_label" {
  name        = "example_label"
  description = "example_description"
}

resource "khulnasoft_assurance_custom_script" "khulnasoft_assurance_custom_script" {
  name        = "khulnasoft_assurance_custom_script"
...
resource "khulnasoft_container_runtime_policy" "container_runtime_policy" {
...
resource "khulnasoft_enforcer_groups" "group" {
...
resource "khulnasoft_enforcer_groups" "group-kube_enforcer" {
  ...
  kube_bench_image_name = "registry.khulnasoft.com/kube-bench:v0.6.5"
  # Secret that holds the registry credentials for the Pod Enforcer and kube-bench
  micro_enforcer_secrets_name = "khulnasoft-registry"
...
resource "khulnasoft_firewall_policy" "example_firewall_policy" {
  // Required values
...
resource "khulnasoft_function_assurance_policy" "example_function_assurance_policy" {
  //Required values
...
resource "khulnasoft_function_runtime_policy" "audit_mode_example" {
  # Basic configuration
...
resource "khulnasoft_host_assurance_policy" "advanced" {
...
resource "khulnasoft_host_runtime_policy" "host_runtime_policy" {
  ...
  scope {
    ...
    variables {
      attribute = "khulnasoft.hostgroup"
      value     = "production"
    }
    variables {
...
resource "khulnasoft_image_assurance_policy" "test_image_policy" {
...
resource "khulnasoft_integration_registry" "integration_registry" {
  name                           = "integration_registry"
  type                           = "AWS"
  advanced_settings_cleanup      = false
  always_pull_patterns           = [":latest", ":v1"]
  author                         = "khulnasoft@khulnasoft.com"
  auto_cleanup                   = false
  auto_pull                      = true
...
resource "khulnasoft_kubernetes_assurance_policy" "example_kubernetes_assurance_policy" {
...
resource "khulnasoft_notification_slack" "example" {
  ...
  main_text   = "KhulnaSoft alert notification"
  name        = "slack-alert"
  service_key = ""
}
...
resource "khulnasoft_role" "example" {
  role_name   = "ExampleTeam"
  description = "Role for ExampleTeam with limited access"
  permission  = khulnasoft_permission_set_saas.example.name
  scopes      = ["Global"]
}

resource "khulnasoft_role_mapping_saas" "example" {
  saml_groups = ["Engineering", "Security"]
  csp_role    = khulnasoft_role.example.role_name
}

resource "khulnasoft_scanner_group" "example" {
  name               = "terraformTest"
  type               = "remote"
  os_type            = "linux"
...
resource "khulnasoft_service" "example_service" {
  name               = "svc_example"
...
resource "khulnasoft_vmware_assurance_policy" "example_vmware_assurance_policy" {
  application_scopes = ["Global"]
...
data "khulnasoft_application_scope_saas" "saas" {
  name = "Global"
}

output "application_scope_saas" {
  value = data.khulnasoft_application_scope_saas.saas
}

output "name" {
  value = data.khulnasoft_application_scope_saas.saas.name
}

output "categories" {
  value = data.khulnasoft_application_scope_saas.saas.categories
}

data "khulnasoft_khulnasoft_api_keys" "single" {
  id = khulnasoft_khulnasoft_api_key.terraform_api_key.id
}

output "api_key_single_id" {
  value = data.khulnasoft_khulnasoft_api_keys.single.apikeys[0].id
}
output "group_id" {
  value = data.khulnasoft_khulnasoft_api_keys.single.apikeys[0].group_id
}
output "expiration" {
  value = data.khulnasoft_khulnasoft_api_keys.single.apikeys[0].expiration
}
output "permission_ids" {
  value = data.khulnasoft_khulnasoft_api_keys.single.apikeys[0].permission_ids
}
output "roles" {
  value = data.khulnasoft_khulnasoft_api_keys.single.apikeys[0].roles
}

output "secret" {
  value     = khulnasoft_khulnasoft_api_key.terraform_api_key.secret
  sensitive = true
}

data "khulnasoft_khulnasoft_api_keys" "list" {
  limit  = 10
  offset = 0
}

output "api_key_list" {
  value     = data.khulnasoft_khulnasoft_api_keys.list
  sensitive = true
}

data "khulnasoft_khulnasoft_labels" "khulnasoft_labels" {}

output "khulnasoft_labels_scope" {
  value = data.khulnasoft_khulnasoft_labels.khulnasoft_labels
}

data "khulnasoft_assurance_custom_script" "example" {
  script_id = "ID of the custom script"
}

output "assurance_custom_script_name" {
  value = data.khulnasoft_assurance_custom_script.example.name
}
...
data "khulnasoft_notifications" "slack-example" {}

output "slack-notification" {
  value = data.khulnasoft_notifications.slack-example
}
...
data "khulnasoft_scanner_group" "all" {}

output "scanner_group_names" {
  value = [for sg in data.khulnasoft_scanner_group.all.scanner_groups : sg.name]
}

data "khulnasoft_permissions_sets_saas" "saas" {}

output "data_permissions_sets_saas" {
  value = data.khulnasoft_permissions_sets_saas.saas
}

output "data_permissions_sets_names_saas" {
  value = data.khulnasoft_permissions_sets_saas.saas[*].permissions_sets[*].name
}

output "data_dashboard_permissions_saas" {
  value = [
    for ps in data.khulnasoft_permissions_sets_saas.saas.permissions_sets : ps.name
    if contains(ps.actions, "cnapp.dashboards.read")
  ]
}