...
resource "khulnasoft_integration_registry" "demoregistry" {
  name = "terraform-ecr"
  url = "us-east-1"
  type = "AWS"
  username = "APIKEY"
...
resource "khulnasoft_firewall_policy" "test-policy" {
...
resource "khulnasoft_service" "test-svc" {
  name = "test-svc"
...
resource "khulnasoft_enforcer_groups" "new" {
  group_id = "terraform"
...
resource "khulnasoft_notification_slack" "new" {
  name = "Slack"
  enabled = true
  type = "slack"
  channel = "#general"
...
resource "khulnasoft_container_runtime_policy" "test" {
  name                          = "test"
...
resource "khulnasoft_image_assurance_policy" "newiap" {
    name = "testprovider"
...
resource "khulnasoft_application_scope" "terraformiap" {
  description = "test123"
  name        = "test18"
