...
func dataSourceEnforcerGroup() *schema.Resource {
	return &schema.Resource{
		Description: "The data source `khulnasoft_enforcer_groups` provides an Enforcer group template that generates a configuration file, which is subsequently used to generate one or more Enforcers using a Docker command.",
		...
		Schema: map[string]*schema.Schema{
			...
			"enforcer_image_name": {
				Type:        schema.TypeString,
				Description: "The specific Khulnasoft Enforcer product image (with image tag) to be deployed.",
				Computed:    true,
			},
			"description": {
				Type:        schema.TypeString,
				Description: "A description for the Khulnasoft Enforcer group.",
				Computed:    true,
			},
			"gateway_name": {
				Type:        schema.TypeString,
			...
			"forensics": {
				Type:        schema.TypeBool,
				Description: "Select Enabled to send activity logs in your containers to the Khulnasoft Server for forensics purposes.",
				Optional:    true,
			},
			"host_forensics": {
				Type:        schema.TypeBool,
				Description: "Select Enabled to send activity logs in your host to the Khulnasoft Server for forensics purposes.",
				Optional:    true,
			},
			"host_network_protection": {
				Type:        schema.TypeBool,
				Description: "When set to `True` applies Firewall Policies to hosts, and allows recording network maps for Khulnasoft services. The Network Firewall setting must be disabled when deploying the Khulnasoft Enforcer on a machine running Rocky Linux. See https://docs.khulnasoft.com/docs/platform-support-limitations-rocky-linux for further information",
				Computed:    true,
			},
			"user_access_control": {
				Type:        schema.TypeBool,
				Description: "When set to `True` applies User Access Control Policies to containers. Note that Khulnasoft Enforcers must be deployed with the KHULNASOFT_RUNC_INTERCEPTION environment variable set to 0 in order to use User Access Control Policies.",
				Computed:    true,
			},
			"image_assurance": {
				Type:        schema.TypeBool,
			...
			"khulnasoft_version": {
				Type:        schema.TypeString,
				Description: "Khulnasoft server version",
				Computed:    true,
				Elem: &schema.Schema{
					Type: schema.TypeString,
				},
			...
			"auto_discover_configure_registries": {
				Type:        schema.TypeBool,
				Description: "This option is available only if `Enable workload discovery` is selected. If selected, the KubeEnforcer will add previously unknown image registries from the cluster to Khulnasoft.",
				Computed:    true,
			},
			...
			"auto_copy_secrets": {
				Type:        schema.TypeBool,
				Description: "This option is applicable only if `Enable Pod Enforcer injection` is selected. Select this option if you want Khulnasoft Enterprise to copy the secrets defined above to the Pod Enforcer namespace and container. Otherwise, you can choose to copy these secrets by other means.",
				Computed:    true,
			},
			...
			"kube_bench_image_name": {
				Type: schema.TypeString,
				Description: `See https://docs.khulnasoft.com/docs/securing-kubernetes-applications#section-configuration-hardening, The KubeEnforcer can deploy the Khulnasoft Security kube-bench open-source product to perform Kubernetes CIS benchmark testing of nodes.
			...
			"gateways": {
				Type:        schema.TypeList,
				Description: "List of Khulnasoft gateway IDs for the Enforcers.",
				Computed:    true,
				Elem: &schema.Schema{
					Type: schema.TypeString,
				},
...
func dataEnforcerGroupRead(d *schema.ResourceData, m interface{}) error {
	...
	if err == nil {
		...
		d.Set("khulnasoft_version", group.KhulnasoftVersion)
