...
const (
   resourceName = "khulnasoft_permission_set_saas.new"
   maxNameLength = 20
...
var invalidConfigTestCases = []struct {
...
}{
   {
       ...
       config: `
           resource "khulnasoft_permission_set_saas" "invalid" {
               description = "test"
           }`,
       errorMsg: "name",
   },
...
func testAccCheckKhulnasoftPermissionSetSaas(name, description string, actions []string) string {
   actionsStr := ""
   ...
   return fmt.Sprintf(`
   resource "khulnasoft_permission_set_saas" "new" {
       name        = "%s"
       description = "%s"
       actions     = [%s]
...
func testAccCheckKhulnasoftPermissionSetSaasExists(n string) resource.TestCheckFunc {
...
func testAccPermissionSetSaasDestroy(s *terraform.State) error {
   ...
   for _, rs := range s.RootModule().Resources {
       if rs.Type != "khulnasoft_permission_set_saas.new" {
           continue
       }
...
func TestKhulnasoftPermissionSetSaasManagement(t *testing.T) {
   if !isSaasEnv() {
   ...
   resource.Test(t, resource.TestCase{
       ...
       Steps: []resource.TestStep{
           {
               Config: testAccCheckKhulnasoftPermissionSetSaas(name, description, extendedTestActions),
               Check: resource.ComposeTestCheckFunc(
                   testAccCheckKhulnasoftPermissionSetSaasExists(resourceName),
               ),
           },
           {
               Config: testAccCheckKhulnasoftPermissionSetSaas(name, "Updated "+description, extendedTestActions),
               Check: resource.ComposeTestCheckFunc(
                   testAccCheckKhulnasoftPermissionSetSaasExists(resourceName),
...
func TestKhulnasoftPermissionSetSaasInvalidConfig(t *testing.T) {
...
func TestKhulnasoftPermissionSetSaasWithExternalChanges(t *testing.T) {
   if !isSaasEnv() {
       t.Skip("Skipping permission set test - not a SaaS environment")
   }
   ...
   resource.Test(t, resource.TestCase{
       ...
       Steps: []resource.TestStep{
           {
               Config: testAccCheckKhulnasoftPermissionSetSaas(name, initialDescription, defaultTestActions),
           ...
           {
               Config: testAccCheckKhulnasoftPermissionSetSaas(name, initialDescription, append(defaultTestActions, "cspm.cloud_accounts.write")),
...
func TestKhulnasoftPermissionSetSaasReadErrorHandling(t *testing.T) {
	if !isSaasEnv() {
		t.Skip("Skipping permission set test - not a SaaS environment")
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckKhulnasoftPermissionSetSaas(name, "test description", defaultTestActions),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckKhulnasoftPermissionSetSaasExists(resourceName),
...
func TestKhulnasoftPermissionSetSaasUpdateErrorHandling(t *testing.T) {
	if !isSaasEnv() {
		t.Skip("Skipping permission set test - not a SaaS environment")
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckKhulnasoftPermissionSetSaas(name, "initial", defaultTestActions),
			},
			{
				Config: testAccCheckKhulnasoftPermissionSetSaas(name, "updated", append(defaultTestActions, "invalid.action")),
...
func TestKhulnasoftPermissionSetSaasValues(t *testing.T) {
    if !isSaasEnv() {
        t.Skip("Skipping permission set test - not a SaaS environment")
    }
