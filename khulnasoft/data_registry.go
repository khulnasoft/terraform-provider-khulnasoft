...
func dataSourceRegistry() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"auto_cleanup": {
				Type:        schema.TypeBool,
				Description: "Automatically clean up images and repositories which are no longer present in the registry from Khulnasoft console",
				Computed:    true,
			},
			...
			"webhook": {
				Type:        schema.TypeSet,
				Optional:    true,
				Description: "When enabled, registry events are sent to the given Khulnasoft webhook url",
