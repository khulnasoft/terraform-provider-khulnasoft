...
func TestKhulnasoftEnforcerGroupResource(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: CheckDestroy(fmt.Sprintf("khulnasoft_enforcer_groups.%v", basicEnforcerGroup.ID)),
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      fmt.Sprintf("khulnasoft_enforcer_groups.%s", basicEnforcerGroup.ID),
				ImportState:       true,
...
func getBasicEnforcerGroupResource(enforcerGroup client.EnforcerGroup) string {
	return fmt.Sprintf(`
	resource "khulnasoft_enforcer_groups" "%s" {
		group_id = "%s"
		description = "%s"
		logical_name = "%s"
		enforce = "%v"
...
func enforcerGroupsRef(name string) string {
	return fmt.Sprintf("khulnasoft_enforcer_groups.%v", name)
}
