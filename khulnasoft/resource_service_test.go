...
func TestResourceKhulnasoftServiceBasicCreate(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: CheckDestroy("khulnasoft_service.test-basic-svc"),
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "khulnasoft_service.test-basic-svc",
				ImportState:       true,
...
func TestResourceKhulnasoftServiceComplexCreate(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Providers:    testAccProviders,
		CheckDestroy: CheckDestroy("khulnasoft_service.test-complex-svc"),
		Steps: []resource.TestStep{
			{
...
func TestResourceKhulnasoftServiceUpdate(t *testing.T) {
	rootRef := serviceResourceRef("test-basic-svc")
...
func serviceResourceRef(name string) string {
	return fmt.Sprintf("khulnasoft_service.%s", name)
}

func getBasicServiceResource() string {
	return fmt.Sprintf(`
	resource "khulnasoft_service" "test-basic-svc" {
		name = "%s"
		description = "%s"
		application_scopes = [
			"%s",
...
func getComplexServiceResource() string {
	return fmt.Sprintf(`
	resource "khulnasoft_firewall_policy" "test" {
		name = "%s"
		description = "this is created for the unit test of service resource"
	}

	resource "khulnasoft_service" "test-complex-svc" {
		name = "%s"
		description = "%s"
		application_scopes = [
			"%s"
		...
		policies = [
			"%s",
			khulnasoft_firewall_policy.test.id
		]
		priority = "%d"
		target = "%s"
		enforce = "%v"
