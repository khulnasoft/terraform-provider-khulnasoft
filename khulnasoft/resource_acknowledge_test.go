...
func TestKhulnasoftAcknowledge(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: CheckDestroy("khulnasoft_acknowledge.terraformap"),
		Steps: []resource.TestStep{
			{
				Config: testAccCheckAcknowledge(description, []map[string]interface{}{issues, issues2}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAcknowledgeExists("khulnasoft_acknowledge.terraformap"),
				),
			},
			//{
			//	Config: testAccCheckAcknowledge(description, []map[string]interface{}{issues, issues2}),
			...
			//{
			//	ResourceName:      "khulnasoft_acknowledge.terraformap",
			//	ImportState:       true,
...
func testAccCheckAcknowledge(comment string, issues []map[string]interface{}) string {
	...
	x := fmt.Sprintf(`
	resource "khulnasoft_acknowledge" "terraformap" {
		comment = "%s"
		%s
