...
func TestKhulnasoftAcknowledge(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: CheckDestroy("khulnasoft_acknowledge.acknowledge"),
		Steps: []resource.TestStep{
			{
				Config: testAccCheckAcknowledgeFullConfig(comment, []map[string]interface{}{issue}),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAcknowledgeExists("khulnasoft_acknowledge.acknowledge"),
					testAccCheckImageExists("khulnasoft_image.example_khulnasoft_image"),
				),
			},
		},
	})
...
resource "khulnasoft_acknowledge" "acknowledge" {
  comment = "%s"
  %s
  depends_on = [khulnasoft_image.example_khulnasoft_image]
}
