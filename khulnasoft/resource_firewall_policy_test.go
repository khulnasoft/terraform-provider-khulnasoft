...
func TestResourceKhulnasoftFirewallPolicy(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: CheckDestroy(fmt.Sprintf("khulnasoft_firewall_policy.%v", basicFirewallPolicy.Name)),
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      fmt.Sprintf("khulnasoft_firewall_policy.%s", basicFirewallPolicy.Name),
				ImportState:       true,
...
func TestResourceFirewallPolicyComplex(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Providers:    testAccProviders,
		CheckDestroy: CheckDestroy(fmt.Sprintf("khulnasoft_firewall_policy.%v", complexFirewallpolicy.Name)),
		Steps: []resource.TestStep{
			{
...
func firewallPolicyResourceRef(name string) string {
	return fmt.Sprintf("khulnasoft_firewall_policy.%s", name)
}

func getBasicFirewallPolicyResource(firewallPolicy client.FirewallPolicy) string {
	return fmt.Sprintf(`
	resource "khulnasoft_firewall_policy" "%s" {
		name = "%s"
		description = "%s"
	}`, firewallPolicy.Name,
		firewallPolicy.Name,
