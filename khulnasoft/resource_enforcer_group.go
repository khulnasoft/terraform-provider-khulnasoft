...
func resourceEnforcerGroup() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"description": {
				Type:        schema.TypeString,
				Description: "A description of the Khulnasoft Enforcer group.",
				Optional:    true,
			},
			"disconnected_count": {
				Type:        schema.TypeInt,
			...
			"micro_enforcer_secrets_name": {
				Type:        schema.TypeString,
				Description: "You can specify the name of the secret (in the Khulnasoft namespace) that Khulnasoft copies into the Pod Enforcer namespace and kube-bench, allowing them access to the Pod Enforcer and kube-bench product images, respectively.",
				Optional:    true,
				Computed:    true,
			},
			"neg_vulns": {
			...
			"user_access_control": {
				Type:        schema.TypeBool,
				Description: "Set `True` to apply User Access Control Policies to containers. Note that Khulnasoft Enforcers must be deployed with the KHULNASOFT_RUNC_INTERCEPTION environment variable set to 0 in order to use User Access Control Policies.",
				Optional:    true,
			},
		},
	}
...
func resourceEnforcerGroupRead(d *schema.ResourceData, m interface{}) error {
	...
	d.Set("khulnasoft_version", r.KhulnasoftVersion)
...
func expandEnforcerGroup(d *schema.ResourceData) client.EnforcerGroup {
	...
	khulnasoftVersion, ok := d.GetOk("khulnasoft_version")
	if ok {
		enforcerGroup.KhulnasoftVersion = khulnasoftVersion.(string)
	}
