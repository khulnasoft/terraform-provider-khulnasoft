...
func TestKhulnasoftHostAssurancePolicy(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: CheckDestroy("khulnasoft_host_assurance_policy.terraformiap"),
		Steps: []resource.TestStep{
			{
				Config: testAccCheckHostAssurancePolicy(description, name, application_scopes),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckHostAssurancePolicyExists("khulnasoft_host_assurance_policy.terraformiap"),
				),
			},
			{
				ResourceName:      "khulnasoft_host_assurance_policy.terraformiap",
				ImportState:       true,
...
func testAccCheckHostAssurancePolicy(description string, name string, application_scopes string) string {
	return fmt.Sprintf(`
	resource "khulnasoft_host_assurance_policy" "terraformiap" {
		description = "%s"
		name = "%s"
		application_scopes = [
			"%s"
